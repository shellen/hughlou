...
export default function VideoCard({
    ...
    return (
      <Link href={`/watch/${rkey}`} aria-label={`Watch: ${title} (${formatDuration(duration)})`} className="block py-2">
        <div className="group flex gap-3">
          <div className="relative w-44 shrink-0 aspect-video rounded-md overflow-hidden bg-[#111113]">
  ...
  return (
    <Link href={`/watch/${rkey}`} aria-label={`Watch: ${title} (${formatDuration(duration)})`}>
      <div className="group">
        <div className="relative aspect-video rounded-lg overflow-hidden bg-[#111113] mb-3">
          <Thumbnail title={title} thumbDataUrl={thumbDataUrl} />
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all" aria-hidden="true" />
        ...
        <h3 className="text-sm font-medium text-[#ededef] group-hover:text-white transition-colors line-clamp-2 leading-snug">
          {title}
        </h3>
        <p className="text-[11px] text-[#8b8b96] mt-1.5 font-mono">{formatRelativeTime(createdAt)}</p>
      </div>
    </Link>
  )
}
